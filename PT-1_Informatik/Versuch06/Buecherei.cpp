...
/* Durch setzten dieser Variable, können Sie entscheiden, welche Includes alle eingebunden werden sollen
 * Bei setzten auf true, sind alle abgeleiteten Klassen (Buch, DVD, Magazin) vorhanden
 * Bei setzten auf false, ist nur die Basisklasse Medium vorhanden
 */
...
void ausgeliehenMedienAusgeben(std::vector<Medium*>& medien);
...
int main()
{
	...
	do  // Anzeige des Menüs
	{
		...
		case '6':
		{
			ausgeliehenMedienAusgeben(medien);
			break;
		}
		case '7':
		{
...
void operator<<(std::ostream& aus, Medium& a)
{
	a.ausgabe();
